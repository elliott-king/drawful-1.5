...
function handleDrawingPrompt(drawing, game_id) {
...
function hasAllPromptsPollCycle(drawing, game_id) {
  // todo: verify we don't go too deep
  drawingHasAllPrompts(drawing, game_id)
    .then(is_done => {
      if (is_done) {
        console.log('Round is done. fixme: call next section of turn')
      }
      else {
        // wait 3 seconds & try again
        setTimeout(() => hasAllPromptsPollCycle(drawing, game_id), 3000)
      }
    })
}
